...
- Added `workflow_dispatch` event to the GitHub Actions workflow in `.github/workflows/changelog.yml`.
- Added a new section titled "TEST CHANGE" in `README.md`.
...
- Introduced a new set for tracking unreleased commit hashes in `process-changelog-chunked.py` to improve metadata handling.
- Added comments in the code to clarify the purpose of new logic regarding version tracking and unreleased entries.
- Enhanced the changelog generation process to include a placeholder for versions without entries, ensuring clarity in the changelog output.
- Added scripts for generating prompts for backfill and update changelogs.
- Implemented a new script for processing changelogs in chunks to handle larger requests.
- Added `helpers/colorUtils.js` to consolidate color-related utility functions previously scattered across multiple files.
...
- Modified the `format_changelog` function in `process-changelog-chunked.py` to accept an additional parameter `unreleased_commit_hashes` for tracking unreleased commits.
- Enhanced the `parse_and_merge_entries` function to return both formatted text and a set of commit hashes.
- Modified the `merge_changelog_entries` function to include commit hash metadata in the changelog for both unreleased and released entries.
- Changed the title format in `changelog.yml` to include the GitHub run number for automated changelog updates.
...
- Modified output details in the changelog summary to specify "Source branch" instead of just "Branch".
- Improved error handling in the changelog generation process to ensure proper exit codes and messages.
- Updated GitHub Actions workflow for changelog generation to use `peter-evans/create-pull-request@v7` for better pull request management.
- Refactored changelog generation process to include automated AI-powered analysis of commits.
- Converted `helpers/genericImageGenerator.js` to `generators/genericImageGenerator.js` and updated import paths for utilities.
- Converted `helpers/logoGenerator.js` to `generators/logoGenerator.js` and updated import paths for utilities.
- Converted `helpers/thumbnailGenerator.js` to `generators/thumbnailGenerator.js` and updated import paths for utilities.
- Updated import paths in `providers/ESPNProvider.js` to reflect the new location of utility functions.
- Updated import paths in `providers/TheSportsDBProvider.js` to reflect the new location of utility functions.
- Modified `routes/cover.js`, `routes/leaguecover.js`, `routes/leaguethumb.js`, `routes/logo.js`, `routes/ncaa.js`, and `routes/thumb.js` to remove unnecessary imports and adjust to the new structure.
...
- Resolved issues in the `process_in_chunks` function to ensure that unreleased commits are correctly tracked and assigned to the appropriate version.
- Fixed the logic for handling batch comments to ensure that entries are correctly assigned to versions based on metadata, even when AI fails to split them by version headers.
- Corrected the handling of entry blocks to ensure proper categorization and merging of changelog entries in `process-changelog-chunked.py`.
...
- Added a cleanup step in the workflow to remove temporary files generated during the changelog processing.
